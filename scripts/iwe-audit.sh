...
if [ ! -d "$DS_DIR/.git" ]; then
    ...
    # (1) /Users/anastasia/Documents/IWE/FMT-strategy-template/ — отдельная директория (авторская)
    # (2) /Users/anastasia/Documents/IWE/FMT-exocortex-template/templates/strategy-skeleton/ — внутри FMT (приезжает через update.sh)
...
"
    ...
    for target in $LEAK_TARGETS; do
        ...
        HITS=$(grep -rIl --include='*.sh' --include='*.env' --include='*.service' \
            ...
            -e "/Users/anastasia" -e "{{CLAUDE_PROJECT_SLUG}}" \
    ...
    if [ $LEAK_COUNT -gt 0 ]; then
        echo "**Найдено $LEAK_COUNT файлов с macOS-путями.** Решение: заменить на \`\$HOME\`, \`/Users/anastasia\` или \`\$IWE_ROOT\`. Возможные молчаливые сбои в скриптах."
        OPTIONAL_MISSING=$((OPTIONAL_MISSING + 1))
    else
