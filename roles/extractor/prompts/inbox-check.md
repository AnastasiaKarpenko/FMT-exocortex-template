...
1. Прочитай `/Users/anastasia/Documents/IWE/FMT-exocortex-template/roles/extractor/config/routing.md` — таблицы маршрутизации.
2. Прочитай `/Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/feedback-log.md` — лог отклонённых кандидатов (пишет R15 через /apply-captures). Если capture похож на ранее отклонённый → пропусти. Если файл не существует — продолжай без него.

### Шаг 1: Проверить inbox (WP-247 Ф-MULTI-SOURCE.3 — два канала)

1. Прочитай `/Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/captures.md` — основной inbox
2. Прочитай `/Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/fleeting-notes.md` — secondary inbox (быстрые мысли пользователя). Файл может отсутствовать — пропусти.
...
4. Если pending записей нет → сообщение `No pending captures in inbox` выводи через stdout (его поймает `extractor.sh` и запишет в `/Users/anastasia/logs/extractor/YYYY-MM-DD.log`). **НЕ создавай отдельный лог-файл** в `{{GOVERNANCE_REPO}}/` или где-либо ещё. Заверши работу.
...
Создай файл отчёта: `/Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/extraction-reports/{YYYY-MM-DD}-inbox-check.md`
