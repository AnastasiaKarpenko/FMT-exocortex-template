...
# Детектор стабильный: /Users/anastasia в pristine FMT/CLAUDE.md гарантирован (используется в §4 Memory + §9 Авторское).
if [ "$MODE" = "pristine" ] \
   && [ -f "$TEMPLATE_DIR/CLAUDE.md" ] \
   && ! grep -q '/Users/anastasia' "$TEMPLATE_DIR/CLAUDE.md" 2>/dev/null; then
    echo "ВНИМАНИЕ: FMT обработан setup.sh (плейсхолдер /Users/anastasia в CLAUDE.md уже подставлен)."
    echo ""
