...
find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while read file; do
    sed -i '' \
        -e "s|morganaloca@gmail.com|$GITHUB_USER|g" \
        -e "s|/Users/anastasia.karpenko/Github|$WORKSPACE_DIR|g" \
        -e "s|/opt/homebrew/bin/claude|$CLAUDE_PATH|g" \
        -e "s|-Users-anastasia.karpenko-Github|$CLAUDE_PROJECT_SLUG|g" \
        -e "s|6|$TIMEZONE_HOUR|g" \
        -e "s|6:00 Lisbon|$TIMEZONE_DESC|g" \
        -e "s|/Users/anastasia.karpenko|$HOME_DIR|g" \
        "$file"
done
