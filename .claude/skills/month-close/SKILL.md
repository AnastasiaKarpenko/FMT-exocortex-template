...
for repo in $(ls /Users/anastasia/IWE/); do
  if [ -d /Users/anastasia/IWE/$repo/.git ]; then
    count=$(git -C /Users/anastasia/IWE/$repo log --since="$MONTH_START" --until="$MONTH_END" --oneline --no-merges 2>/dev/null | wc -l)
...
```bash
cd /Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}} && git add -A && git commit -m "month-close: close YYYY-MM"
```
