...
**Коммиты:**
```bash
for repo in $(ls /Users/anastasia/Documents/IWE/); do
  if [ -d /Users/anastasia/Documents/IWE/$repo/.git ]; then
    commits=$(git -C /Users/anastasia/Documents/IWE/$repo log --since="last monday 00:00" --until="today 00:00" --oneline --no-merges 2>/dev/null)
...
**Календарь недели:**
```bash
bash /Users/anastasia/Documents/IWE/scripts/server-calendar.sh --week $(date -v-mon +%Y-%m-%d 2>/dev/null || date -d "last monday" +%Y-%m-%d)
```
...
Скрипт обходит все `/Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/WP-*.md` со `status: in_progress` (или без явного status), извлекает строки Ф-таблицы со статусом `⏳ pending` / `pending`, выводит сводку формата:
...
Если скрипта нет — fallback: `grep -l "status: in_progress" /Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}}/inbox/WP-*.md` → для каждого `grep -E "⏳.*pending|Ф[0-9]+.*pending"`.
...
```bash
echo "=== distinctions.md ===" && wc -l /Users/anastasia/Documents/IWE/.claude/rules/distinctions.md
...
```bash
cd /Users/anastasia/Documents/IWE/{{GOVERNANCE_REPO}} && git add -A && git commit -m "week-close: W{N} итоги q:{score}" && git push
```
